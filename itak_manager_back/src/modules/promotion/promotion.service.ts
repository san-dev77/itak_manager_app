...
export class PromotionService {
  ...
  async getNextClass(currentClassId: string): Promise<NextClassResponseDto> {
    try {
      ...
      throw new Error(
        `Erreur lors de la recherche de la classe supérieure: ${message}`,
      );
    }
  }
    ...
    try {
      ...
      if (!currentStudentClass) {
        throw new NotFoundException(
          'Aucune classe actuelle trouvée pour cet étudiant',
        );
      }
      ...
      const savedPromotion =
        await this.studentPromotionRepository.save(promotion);
  ...
  async bulkPromoteClass(
    bulkPromotionDto: BulkPromotionDto,
  ): Promise<PromotionResponseDto[]> {
    try {
      ...
      if (studentClasses.length === 0) {
        throw new NotFoundException(
          'Aucun étudiant trouvé dans la classe source',
        );
      }
  ...
  async getPromotionHistory(
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Erreur inconnue';
      throw new Error(
        `Erreur lors de la récupération de l'historique: ${message}`,
      );
    }
  }
